...
it('can change star name', async () => {
    let instance = await StarNotary.deployed();
    let initialStarName = await instance.starName.call();
    assert.equal(initialStarName, "Awesome Udacity Star");
    await instance.changeName("STAR NAME CHANGED FROM TEST");
    let newStarName = await instance.starName.call();
    assert.equal(newStarName, "STAR NAME CHANGED FROM TEST");
 });
